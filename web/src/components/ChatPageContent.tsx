...
import { Sparkles, Trash2, Send, Loader2, ChevronUp } from 'lucide-react';
...
export default function ChatPageContent({ quickActions }: Props) {
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-[var(--text-muted)] leading-tight">
                {syncing ? 'Sincronizando...' : toolStatus ? toolStatus : isThinking ? 'Escribiendo...' : 'En línea · 24/7'}
              </p>
            </div>
          </div>
          <button
            ...
            className={`p-2 rounded-xl transition-all ${confirmClear ? 'bg-red-500/10 text-red-500 hover:bg-red-500/20' : 'text-[var(--text-muted)] hover:text-[var(--foreground)] hover:bg-[var(--surface-secondary)]'}`}
          >
            <Trash2 size={18} />
          </button>
        </header>
            ...
            <div className="flex flex-col items-center justify-center py-16 gap-4 text-center">
              ...
              <div>
                ...
                <p className="text-sm text-[var(--text-muted)] mt-1 max-w-xs">
                  Usá el chat para gestionar el club sin abrir ninguna pantalla.
                </p>
              </div>
            </div>
          )}
            ...
            return (
              <div key={item.key} className={`flex items-end gap-2 mb-1 ${isUser ? 'justify-end' : 'justify-start'}`}>
                ...
                <div className={`max-w-[75%] flex flex-col ${isUser ? 'items-end' : 'items-start'}`}>
                  <div className={`rounded-2xl px-3.5 py-2.5 text-sm leading-relaxed ${isUser ? 'bg-[var(--primary)] text-white rounded-br-sm shadow-md shadow-[var(--primary)]/20' : 'bg-[var(--surface-secondary)] border border-[var(--border)] text-[var(--foreground)] rounded-bl-sm'}`}>
                    {isEmpty ? <TypingDots /> : <span className="whitespace-pre-wrap">{msg.content}</span>}
                  </div>
                  <span className="text-[10px] text-[var(--text-muted)] mt-1 px-1">{formatTime(msg.timestamp)}</span>
                </div>
              </div>
            );
          })}
        ...
        <div className="shrink-0 border-t border-[var(--border)] bg-[var(--surface)] px-4 py-3">
          <div className="flex items-end gap-2">
            ...
            <button
              ...
              style={{ backgroundColor: input.trim() && !isLoading ? 'var(--primary)' : 'var(--surface-secondary)' }}
            >
              {isLoading
